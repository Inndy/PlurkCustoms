...
(function($) {
    ...
    function muteAll() {
        ...
        $.ajax({
            ...
            data: {
                known_friends: JSON.stringify([uid])
            },
            type: "POST",
            cache: false,
            success: function(dataJSON) {
                ...
                for (var i = 0; i < data.unread_plurks.length; i++)
                    if (data.unread_plurks[i].owner_id == uid || data.unread_plurks[i].plurk_type > 1) {
                        data.unread_plurks.splice(i, 1);
                        i--;
                    }
    ...
    setInterval(function() {
        ...
        if (plurkManager.length) {
            ...
            if (plurkManager.find(".mute").length == 0) {
                ...
                var muteBtn = muted ? $("<a class='mute delete unmute' href='#'>" + __("解除消音") + "</a>") : $("<a class='mute delete' href='#'>" + __("消音") + "</a>");
